...
echo "Checking received URL..."

if [[ "$received_url" == *"music.youtube.com"* ]]; then
    youtube_link="www.youtube.com"
    received_url="${received_url/music.youtube.com/"$youtube_link"}"
fi

echo "Retrieving album information..."
# Downloading the json data of the first track only
jsondata=`yt-dlp -j --playlist-items 1 $received_url`
...
yt-dlp  --ignore-errors \
        ...
        -o "~/Music/$artist/%(album)s/%(playlist_index)s - %(title)s.%(ext)s" "$received_url"

mpc rescan | grep "Updating"
